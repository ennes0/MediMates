    ...
    try {
        const contact = await db.get(
            `SELECT c.contact_id, c.contact_user_id, c.contact_type, c.name, 
                    ...
                    u.email, up.name as user_name, up.username, up.profile_picture
             FROM contacts c
             JOIN users u ON c.contact_user_id = u.user_id
             JOIN user_profiles up ON c.contact_user_id = up.user_id
...
router.post('/', verifyToken, async (req, res) => {
    const { userId, type = 'friend', nickname = null } = req.body;
    ...
    try {
        ...
        // Add new contact - use 'name' field instead of 'nickname'
        const result = await db.run(
            `INSERT INTO contacts (user_id, contact_user_id, contact_type, name)
             VALUES (?, ?, ?, ?)`,
            [req.userId, userId, type, nickname]
        );
        ...
        return res.status(500).json(
            formatErrorResponse('Failed to add contact')
        );
    }
});
...
router.put('/:id', verifyToken, async (req, res) => {
    const contactId = req.params.id;
    const { type, nickname, canViewMedications, canViewReminders, canViewHistory } = req.body;
    
    try {
        // Check if contact exists and belongs to the user
        const existingContact = await db.get(
            `SELECT contact_id FROM contacts WHERE contact_id = ? AND user_id = ?`,
            [contactId, req.userId]
        );
        ...
        // Update contact - use 'name' instead of 'nickname'
        await db.run(
            `UPDATE contacts SET 
            ...
            [
                type, 
                nickname, 
                canViewMedications !== undefined ? (canViewMedications ? 1 : 0) : null,
                canViewReminders !== undefined ? (canViewReminders ? 1 : 0) : null,
                canViewHistory !== undefined ? (canViewHistory ? 1 : 0) : null,
                contactId,
                req.userId
            ]
        );
