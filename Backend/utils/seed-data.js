                      ...
                      db.run(
                        ...
                        function(err) {
                          ...
                          if (schedule.times && schedule.times.length > 0) {
                            let completedTimes = 0;                              schedule.times.forEach(time => {
                              db.run(
      ...
      db.serialize(() => {
        ...
        db.run(
          `INSERT INTO reminders (user_id, date, time, title, created_at, updated_at)
           VALUES (?, ?, ?, 'Medication Reminder', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
          [userId, reminder.date, reminder.time],
          function(err) {
            if (err) {
            ...
            // Insert reminder medications
            let completedMeds = 0;
            
            reminder.medications.forEach(med => {
              db.run(
                `INSERT INTO reminder_medications (reminder_id, medication_id, status, created_at)
                 VALUES (?, ?, ?, CURRENT_TIMESTAMP)`,
